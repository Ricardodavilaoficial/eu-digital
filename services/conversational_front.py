...
2. Para mensagem genérica, social, vaga ou sobre assunto externo, responda como abertura social: cumprimente, faça uma reação curta e simpática à mensagem, sem afirmar fatos externos, e faça uma ponte curta para o MEI Robô.
3. Diga que o MEI Robô ajuda quem atende pelo WhatsApp a responder melhor, organizar demandas, ganhar tempo e gerar mais resultado.
4. Termine com frase afirmativa e ponto final.
    ...
    try:
        ...
        if raw_unqualified_lead_discovery_state:
            ...
            discovery_contract_block = (
                ...
                "validade_factual=usar_apenas_informacao_confirmada_no_contexto\n"
                "reacao_social=curta_neutra_leve_baseada_no_tom_sem_tratar_assunto_externo_como_fato\n"
                "ponte_institucional=valor_curto_do_mei_robo_no_whatsapp_para_organizar_contatos_conversas_e_proximo_passo\n"
                "corpo_util=ponte_institucional_sem_pergunta_de_nome_ou_segmento\n"
                "identidade=runtime_finalizer_controla_nome_e_atividade_segmento\n"
                f"dados_faltantes={','.join(_discovery_missing) if _discovery_missing else 'nenhum'}\n"
                "se_faltam_nome_e_atividade=runtime_finalizer_pede_os_dois_na_mesma_frase\n"
                "se_ja_tem_nome=runtime_finalizer_pede_atividade_segmento\n"
                "se_ja_tem_atividade_segmento=runtime_finalizer_pede_nome\n"
        ...
        if free_mode and next_step != "SEND_LINK":
            ...
            try:
                if str(reply_source or "").strip() == "front_structured_python_assembly":
                    ...
                    if _technical_direct_platform_kb:
                        ...
                        # - nao cria abertura;
                        # - nao usa frase fixa;
                        # - nao usa palavra-chave;
                        # - nao mexe em prompt;
                        # - nao chama LLM;
                        # - preserva a abertura que ja veio do fluxo;
                        # - so reequilibra abertura + microcena quando a
                        #   micro_scene_conversational inteira esta presente
                        #   na resposta antes do corte final.
                        try:
                            _factory_scene = str(
                            ...
                            if _factory_scene_guard:
                                ...
                                if _scene_len > 680:
                                ...
                                elif _reply_now and _factory_scene in _reply_now:
                                    ...
                                    # Se a microcena ja esta completa e a resposta cabe,
                                    # preserva exatamente a abertura gerada pelo fluxo.
                                    if len(_reply_now) <= _max_total and not _tail:
                                        _reason = "already_complete"

                                    # Se ha abertura antes da microcena e a resposta
                                    # ultrapassou o orcamento, reduz apenas a abertura.
                                    elif _head:
                                        _head_fit = _head
                                        if len(_head_fit) > _head_budget:
                                            _head_fit = _front_trim_to_complete_sentence(
                                                _head_fit,
                                                _head_budget,
                                            ).strip()

                                        _candidate = (
                                            f"{_head_fit} {_factory_scene}".strip()
                                            if _head_fit
                                            else _factory_scene
                                        )
                                        ...
                                        if len(_candidate) <= _max_total:
                                            ...
                                            _applied = True
                                            _reason = "existing_head_rebudgeted"
                                        else:
                                            _reason = "candidate_over_budget"

                                    else:
                                        # Sem abertura existente, a guarda nao inventa nada.
                                        # Deixa o fluxo normal seguir.
                                        _reason = "no_existing_head"
                                    ...
                                    try:
                                        logging.info(
                                            "[FRONT_FACTORY_MICRO_SCENE_BUDGET] applied=False reason=scene_not_present scene_len=%s reply_len=%s max_chars=%s",
                                            _scene_len,
                                            len(_reply_now),
                                            _max_total,
                                        )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        def _raw_discovery_body_is_degraded_v1(value: str) -> bool:
                            s = str(value or "").strip()
                            if not s:
                                return True
                            ...
                            # Critério estrutural, não semântico:
                            # depois do prune, uma resposta muito curta não sustenta
                            # a primeira fala institucional do MEI Robô.
                            if len(compact) < 80:
                                return True

                            # Outro critério estrutural: corpo de uma única frase muito curta.
                            # Não olha assunto, clima, profissão ou intenção.
                            import re
                            parts = [
                                p.strip()
                        ...
                        if _raw_discovery_degraded_body:
                            try:
                                logging.info(
                                    "[RAW_DISCOVERY_DEGRADED_BODY] detected=True reply_len=%s",
                                    len(str(_free_reply or "")),
                                )
                            except Exception:
                                pass
