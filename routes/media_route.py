...
# Corrigir o caminho do ffmpeg no Render.com
...
def receber_mensagem():
    try:
        ...
        if tipo == "texto" and texto:
            ...
            try:
                print("📥 Convertendo áudio recebido em webm...")
...
def receber_audio():
    try:
        ...
        if "file" not in request.files:
            ...
            return jsonify({"erro": "Arquivo de áudio não encontrado no request."}), 400

        arquivo = request.files["file"]
        print(f"📦 Nome do arquivo recebido: {arquivo.filename}")
        ...
        print("🎧 Convertendo áudio com pydub...")
        audio = AudioSegment.from_file(caminho_webm, format="webm")
        audio = audio.set_frame_rate(16000).set_channels(1).set_sample_width(2)
        audio.export(caminho_wav, format="wav")
