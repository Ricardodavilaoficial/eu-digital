...
# Provider unificado (usa ADC/inline/arquivo conforme env)
from services.gcp_creds import get_firestore_client
# Para o SERVER_TIMESTAMP
from google.cloud import firestore as gcf

voz_process_bp = Blueprint("voz_process_bp", __name__)

def _get_fs_client():
    # mantém a função para mínima mudança
...
def api_voz_process():
    ...
    try:
        ...
        new_vox = {
            ...
            "voiceId": voice_id,
            "updatedAt": gcf.SERVER_TIMESTAMP,  # <— precisa do import alias gcf
        }
        doc_ref.set({"vozClonada": new_vox}, merge=True)

        return jsonify({"ok": True, "status": status, "voiceId": voice_id})
    except Exception as e:
        # devolve erro “amigável” p/ debug rápido via CMD
        return jsonify({"ok": False, "error": "process_failed", "detail": str(e)}), 500
