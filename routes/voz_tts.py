...
# routes/voz_tts.py
# Blueprint: TTS via ElevenLabs (POST /api/voz/tts)
#
# Requisitos de ambiente:
#   ELEVEN_API_KEY (obrigatório)
#   ELEVEN_VOICE_ID (opcional; pode ser passado por query/body como voice_id)
#   VOZ_V2_ENABLED=true (opcional; se presente e != 'true', recusar requisições)
#
# Retorna: audio/mpeg (MP3) contendo a locução do texto informado.
#
# Segurança/Observações:
# - Não armazena nada no Firestore/Storage.
# - Usa timeouts conservadores na chamada externa.
# - Sanitiza tamanho do texto (limite padrão 1000 chars, configurável por env VOZ_TTS_MAX_CHARS).
# - Suporta body JSON {text, voice_id, model, optimize_streaming_latency}
...
def tts_post():
    ...
    # Parâmetros opcionais de modelo
    ...
    payload = {
        ...
        "model_id": model_id,
        "voice_settings": {
            # Valores padrão razoáveis; podem ser tunados futuramente
            ...
            "use_speaker_boost": True if _env_true(os.environ.get("ELEVEN_VOICE_SPK_BOOST","true")) else False
        }
    }
    # Parâmetro opcional de latência de streaming
    try:
    ...
    if r.status_code != 200:
        # Tenta extrair msg de erro da Eleven
        detail = None
        try:
            detail = r.json()
        except Exception:
    ...
    # Cabeçalhos para tocar no navegador/HTML5
    headers_resp = {
...
# Opcional: ping simples do blueprint
