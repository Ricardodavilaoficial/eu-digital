...
def perform_stt_logic(raw: bytes, ctype: str) -> tuple[dict, int]:
    """
    Lógica core do STT desacoplada do Flask.
    Pode ser chamada diretamente por outros módulos, sem HTTP interno.
    Retorna: (payload_dict, http_status_code)
    """

    # Limiar de bytes
    min_bytes_default = 100
    if ctype in ("audio/ogg", "application/ogg", "audio/opus"):
        min_bytes_default = _env_int("STT_MIN_BYTES_OGG", 800)
    ...
    try:
        ...
        return (
            {"ok": False, "error": "speech_lib_missing", "detail": str(e)},
            501,
        )
    ...
    def _make_config(*, enc: str, sr: int | None, model: str | None):
        ...
        if enc == "OGG_OPUS":
            ...
            cfg_kwargs["model"] = model or os.environ.get(
                "STT_MODEL",
                "latest_short",
            )
    ...
    try:
        if encoding == "OGG_OPUS":
            cfg1 = _make_config(
                enc=encoding,
                sr=sample_rate_hz,
                model=os.environ.get("STT_MODEL", "latest_short"),
            )
            ...
            if t1:
                return {"ok": True, "transcript": t1, "confidence": c1}, 200
            ...
            if t2:
                return {"ok": True, "transcript": t2, "confidence": c2}, 200
            ...
            if _env_true(os.environ.get("STT_RETRY_LONG", "false")):
                ...
                if t3:
                    return {"ok": True, "transcript": t3, "confidence": c3}, 200

            payload = {
                "ok": False,
                "error": "empty_transcript",
                "detail": "no_speech_or_too_noisy",
            }
        ...
        if not t:
            ...
            if _env_true(os.environ.get("STT_DEBUG", "false")):
                payload["debug"] = {
                    "ctype": ctype,
                    "bytes": len(raw),
                    "attempts": [{"n": 1, "sr": sample_rate_hz, "len": 0}],
                }
            return payload, 200

        return {"ok": True, "transcript": t, "confidence": c}, 200
...
def stt_post():
    ...
    payload, status_code = perform_stt_logic(raw, ctype)
