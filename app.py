...
# CORS consolidado (apenas uma chamada)
...
# ✅ Blueprints (cada um registrado uma única vez)
app.register_blueprint(verificacao_bp)
if bp_cnpj_publica:
    app.register_blueprint(bp_cnpj_publica)

# NOVOS: Signed URL + Opt-in
app.register_blueprint(media_bp)
app.register_blueprint(contacts_bp)  # <-- correção: registrar contacts_bp
...
# --------------------------------------------------------------------
# 2) Agora que o app existe, importe e registre blueprints auxiliares
...
# >>> Verificação de Autoridade — (registro do gate)
try:
...
try:
    ...
    def wa_bot_health():
        return __jsonify(wa_bot.healthcheck()), 200
except Exception as e:
    # Evita quebrar o app se algo der errado na importação
    @app.get(\"/internal/wa-bot/health\")
    def wa_bot_health_fallback():
        return __jsonify({\"ok\": False, \"error\": str(e), \"stage\": \"route\"}), 200
